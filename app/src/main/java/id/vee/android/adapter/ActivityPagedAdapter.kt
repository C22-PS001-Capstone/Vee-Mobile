...
class ActivityPagedAdapter(private val onItemClick: (Activity) -> Unit) :
    PagingDataAdapter<Activity, MyViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        if(activity != null){
            activity.isMonthShow =
                (position > 0 && activity.date.formatDate("MMM") != getItem(position - 1)?.date?.formatDate(
                    "MMM"
                )) || position == 0
